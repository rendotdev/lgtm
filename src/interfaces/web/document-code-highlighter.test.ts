...
import { DocumentCodeHighlighterClass } from "./document-code-highlighter.ts";

function highlighter(params: { codeToHtml?: typeof codeToHtml } = {}) {
  return new DocumentCodeHighlighterClass(
    {},
    {
...
describe("DocumentCodeHighlighterClass", () => {
  it("reads fenced languages and treats patch blocks as diffs", () => {
    const DocumentCodeHighlighter = highlighter();

    expect(
      DocumentCodeHighlighter.languageFromClassName({ className: "language-typescript" }),
    ).toBe("typescript");
    expect(DocumentCodeHighlighter.languageFromClassName({ className: "language-patch" })).toBe(
      "diff",
    );
    expect(DocumentCodeHighlighter.languageFromClassName({})).toBe("text");
  });
