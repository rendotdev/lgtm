...
import { CliUpdateCommandServiceBuilder, CliUpdaterServiceBuilder } from "./cli-update.ts";

describe("CliUpdateCommandService", () => {
  it("captures successful command output instead of writing through the active terminal", async () => {
    const Runner = CliUpdateCommandServiceBuilder({ config: {}, deps: { spawn } });
...
describe("CliUpdaterService", () => {
  it("updates through the npm executable belonging to the active global installation", async () => {
    ...
    const Updater = CliUpdaterServiceBuilder({
      config: {
  ...
  it("does not reinstall a CLI that is already current", async () => {
    ...
    const Updater = CliUpdaterServiceBuilder({
      config: { packageRoot: "/runtime/lib/node_modules/@rendotdev/lgtm", currentVersion: "0.2.0" },
      deps: {
  ...
  it("skips local and npx installations", async () => {
    const runCommand = vi.fn(async () => "");
    const Updater = CliUpdaterServiceBuilder({
      config: { packageRoot: "/project/node_modules/@rendotdev/lgtm", currentVersion: "0.1.12" },
