...
describe("review domain", () => {
  it("builds review source metadata from changed content", () => {
    const source = ReviewSourceSingleton.build({
      file: {
  ...
  it("preserves existing comments when rebuilding a review", () => {
    const existingReview = ReviewBuilderSingleton.build({
      kind: "diff",
    ...
    const rebuilt = ReviewBuilderSingleton.build({
      ...existingReview,
  ...
  it("formats a completed review for an agent", () => {
    ...
    expect(ReviewFormatterSingleton.format({ review, reviewPath: review.reviewPath })).toContain(
      "Status: approved",
    );
  });
});
