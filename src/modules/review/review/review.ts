...
export const { ReviewSourceSingleton, ReviewSourceSingletonBuilder } = build().singleton(
  "ReviewSourceSingleton",
  {
    build() {
      function buildSource(params: { file: DiffReviewFileInput; index: number }): ReviewSourceFile {
        ...
        return {
          ...
          language: languageFromPath({ location: params.file.location }),
      ...
      function countChangedLines(params: { oldText: string; newText: string }) {
        const oldLines = splitLines({ value: params.oldText });
        const newLines = splitLines({ value: params.newText });
      ...
      function splitLines(params: { value: string }) {
      ...
      function languageFromPath(params: { location: string }) {
...
export const { ReviewBuilderSingleton, ReviewBuilderSingletonBuilder } = build().singleton(
  "ReviewBuilderSingleton",
  {
    build() {
      ...
      return { build: buildReview };
    },
  },
);

export const { ReviewFormatterSingleton, ReviewFormatterSingletonBuilder } = build().singleton(
  "ReviewFormatterSingleton",
  {
    build() {
      function format(params: { review: ReviewJson; reviewPath: string }) {
        ...
        if (review.kind === "document") {
          ...
          for (const comment of comments) {
            ...
            lines.push("");
            lines.push(
              `Selected text: ${truncate({ value: comment.selectedText.trim() || "(none)" })}`,
            );
      ...
      function formatLineRange(params: { comment: ReviewComment }) {
      ...
      function truncate(params: { value: string }) {
