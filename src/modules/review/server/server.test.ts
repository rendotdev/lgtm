...
import type { ReviewJson, ReviewOutcome, ReviewPointer } from "../review/review.ts";
import {
  BuiltCliPathServiceBuilder,
  collectGitReviewFilesSinceLast,
  finishReview,
  openReview,
  stopReview,
  waitForReview,
  WebRootServiceBuilder,
} from "./server.ts";
...
describe("WebRootService", () => {
  it("uses the built frontend before alternate build locations", async () => {
    ...
    const Resolver = WebRootServiceBuilder({
      config: {},
      deps: { modulePath: () => "/project/src/modules/review/server/server.ts", stat },
    });
...
describe("BuiltCliPathService", () => {
  it("uses the built CLI when running from source", async () => {
    const stat = vi.fn(async () => undefined);
    const Resolver = BuiltCliPathServiceBuilder({
      config: { modulePath: "/project/src/modules/review/server/server.ts" },
      deps: { stat },
    });
  ...
  it("uses the packaged CLI when running from the Pi extension entrypoint", async () => {
    const stat = vi.fn(async () => undefined);
    const Resolver = BuiltCliPathServiceBuilder({
      config: { modulePath: "/project/extensions/index.mjs" },
      deps: { stat },
    });
  ...
  it("uses the packaged CLI when running from the legacy Pi extension entrypoint", async () => {
    const stat = vi.fn(async () => undefined);
    const Resolver = BuiltCliPathServiceBuilder({
      config: { modulePath: "/project/dist/pi/lgtm.mjs" },
      deps: { stat },
    });
  ...
  it("uses the current bundled CLI path", async () => {
    const stat = vi.fn(async () => undefined);
    const Resolver = BuiltCliPathServiceBuilder({
      config: { modulePath: "/project/dist/cli.mjs" },
      deps: { stat },
    });
