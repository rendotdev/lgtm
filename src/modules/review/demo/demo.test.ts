...
import { DemoReviewSingleton } from "./demo.ts";

describe("DemoReviewSingleton", () => {
  it("builds a grouped multi-file diff demo", () => {
    const demo = DemoReviewSingleton.create({ kind: "diff" });
  ...
  it("builds a Markdown-rich document demo", () => {
    const demo = DemoReviewSingleton.create({ kind: "document" });
  ...
  it("builds realistic comments for both demo kinds", () => {
    const diffComments = DemoReviewSingleton.createComments({ kind: "diff" });
    const documentComments = DemoReviewSingleton.createComments({ kind: "document" });
