...
import type { DocumentComment, OpenReviewInput, ReviewComment } from "../review/review.ts";
...
export const { DemoReviewSingleton, DemoReviewSingletonBuilder } = build().singleton(
  "DemoReviewSingleton",
  {
    build() {
      function create(params: { kind: DemoReviewKind }): OpenReviewInput {
        return params.kind === "document" ? createDocumentReview() : createDiffReview();
      }

      function createComments(params: { kind: DemoReviewKind }): DemoReviewComments {
            ...
            documentComments: [
              {
                ...
                suffix: "",
                comment:
                  "Could we show the next retry time so users know the task is still active?",
                createdAt: timestamp,
                updatedAt: timestamp,
              },
            ],
      ...
      function createDiffReview(): OpenReviewInput {
        return {
          kind: "diff",
...
`,
      ...
      function createDocumentReview(): OpenReviewInput {
        return {
          kind: "document",
