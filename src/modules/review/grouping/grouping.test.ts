...
import type { ReviewSourceFile } from "../review/review.ts";
import { ReviewGroupingService } from "./grouping.ts";
...
describe("ReviewGroupingService", () => {
  it("preserves authored group and file order", () => {
    expect(
      ReviewGroupingService.build({
        files,
        groups: [
  ...
  it("leaves reviews without authored groups flat", () => {
    expect(ReviewGroupingService.build({ files, groups: undefined })).toBeUndefined();
  });
});
