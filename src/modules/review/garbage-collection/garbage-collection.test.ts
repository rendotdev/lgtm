...
import {
  REVIEW_RETENTION_MILLISECONDS,
  ReviewRetentionServiceBuilder,
} from "../retention/retention.ts";
import { ReviewGarbageCollectionServiceBuilder } from "./garbage-collection.ts";
...
describe("ReviewGarbageCollectionService", () => {
  it("removes expired reviews, including open reviews, and preserves fresh reviews", async () => {
    ...
    const Collector = ReviewGarbageCollectionServiceBuilder({
      config: {},
      deps: {
        retentionPolicy: ReviewRetentionServiceBuilder({
