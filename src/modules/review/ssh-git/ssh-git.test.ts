...
import {
  SSHCommandServiceBuilder,
  SSHControlConnectionServiceBuilder,
  SSHGitRepositoryServiceBuilder,
} from "./ssh-git.ts";

describe("SSHCommandService", () => {
  it("single-quotes remote arguments and preserves apostrophes", () => {
    const Encoder = SSHCommandServiceBuilder({
      config: { maximumCommandLength: 10_000 },
      deps: {},
    });
  ...
  it("rejects NUL bytes and oversized commands", () => {
    const Encoder = SSHCommandServiceBuilder({ config: { maximumCommandLength: 30 }, deps: {} });
...
describe("SSHControlConnectionService", () => {
  it("opens one control connection, reads a framed response, and cleans up", async () => {
    ...
    const Connection = SSHControlConnectionServiceBuilder({
      config: {},
      deps: {
...
describe("SSHGitRepositoryService", () => {
  it("collects tracked and untracked remote changes without remote lgtm", async () => {
    ...
    const Reader = SSHGitRepositoryServiceBuilder({
      config: { fileConcurrency: 2, maximumFileBytes: 1_000_000 },
      deps: {
  ...
  it("requires an absolute remote repository path", async () => {
    const Reader = SSHGitRepositoryServiceBuilder({
      config: { fileConcurrency: 1, maximumFileBytes: 1_000 },
      deps: {
        commandEncoder: SSHCommandServiceBuilder({
          config: { maximumCommandLength: 1_000 },
          deps: {},
        }),
        connection: {
          open: vi.fn(),
