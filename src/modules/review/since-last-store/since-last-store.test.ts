...
import type { ReviewJson, ReviewPayload, ReviewStatus } from "../review/review.ts";
import { ReviewSinceLastStoreService } from "./since-last-store.ts";
...
describe("ReviewSinceLastStoreService", () => {
  it("uses the newest compatible diff review in the repository", async () => {
    ...
    await expect(
      ReviewSinceLastStoreService.collect({
        root,
