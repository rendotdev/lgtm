...
import { PreferencesStoreServiceBuilder } from "./store.ts";
...
async function makeStore() {
  ...
  return {
    cwd,
    store: PreferencesStoreServiceBuilder({ config: { cwd }, deps: {} }),
  };
}
...
describe("PreferencesStoreService", () => {
  ...
  it("writes the config under the repository .lgtm directory", async () => {
    ...
    expect(await readFile(store.getPath({}), "utf8")).toBe(
      '{\n  "diffStyle": "split",\n  "lineWrap": true,\n  "sidebarWidth": 320,\n  "fileExpansion": "collapsed",\n  "fileExpansionOverrides": {\n    "src/example.ts": "expanded"\n  }\n}\n',
    );
  });

  it("preserves comments and unrelated config keys", async () => {
    const { cwd, store } = await makeStore();
    ...
    const source = await readFile(store.getPath({}), "utf8");
