...
import type { OpenReviewInput } from "../../modules/review/review/review.ts";
...
export const { LgtmPiExtensionService, LgtmPiExtensionServiceBuilder } = build().service(
  "LgtmPiExtensionService",
  {
    config: {},
    deps: {
      collectGitReview: GitReviewService.collect,
      finishReview,
      openReview,
      resolvePath: resolve,
      stopReview,
    ...
    build({ deps }) {
      function register(params: { pi: ExtensionAPI }) {
        const pi = params.pi;
        const reviewPaths = new Set<string>();
        registerOpenGitReviewTool(pi, reviewPaths);
        registerOpenWorktreeReviewTool(pi, reviewPaths);
        registerOpenJsonReviewTool(pi, reviewPaths);
        registerOpenDocumentReviewTool(pi, reviewPaths);
        registerFinishReviewTool(pi);
        ...
        pi.on("session_shutdown", async (_event, ctx) => {
          ...
          await Promise.all(
            ownedReviewPaths.map(function stopOwnedReview(reviewPath) {
              return deps.stopReview(ctx.cwd, reviewPath);
            }),
          );
        });
      }
      ...
      async function openFromPi(
        ...
        const pointer = await deps.openReview(input, reviewOptions(pi, reviewPaths, ctx, signal));
      ...
      function registerOpenGitReviewTool(pi: ExtensionAPI, reviewPaths: Set<string>) {
        pi.registerTool({
          ...
          parameters: Type.Object({
            ...
            remoteCwd: Type.Optional(
              Type.String({
                description:
                  "Absolute repository path on the remote machine. Required with remote.",
              }),
            ),
          ...
          execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
            const collection = await deps.collectGitReview({
              cwd: ctx.cwd,
            ...
            return openFromPi(pi, reviewPaths, ctx, signal, {
              kind: "diff",
              name: params.name,
      ...
      function registerOpenWorktreeReviewTool(pi: ExtensionAPI, reviewPaths: Set<string>) {
        pi.registerTool({
          name: "lgtm-open-worktree-review",
          label: "Open Worktree Review",
          description:
            "Open an lgtm browser review for changes in a local or SSH-hosted Git worktree.",
          promptSnippet:
          ...
          execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
            const collection = await deps.collectGitReview({
              cwd: params.remote ? ctx.cwd : deps.resolvePath(ctx.cwd, params.worktree),
              remote: params.remote,
      ...
      function registerOpenJsonReviewTool(pi: ExtensionAPI, reviewPaths: Set<string>) {
        pi.registerTool({
          ...
          execute: async (_toolCallId, params, signal, _onUpdate, ctx) =>
            openFromPi(pi, reviewPaths, ctx, signal, {
              kind: "diff",
              name: params.name,
              files: params.files,
              groups: params.groups,
      ...
      function registerOpenDocumentReviewTool(pi: ExtensionAPI, reviewPaths: Set<string>) {
        pi.registerTool({
      ...
      function registerFinishReviewTool(pi: ExtensionAPI) {
        pi.registerTool({
          ...
          execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
            const result = await deps.finishReview(ctx.cwd, params.reviewPath);
            if (!result.found) {
              return {
...
export default function registerLgtmPiExtension(pi: ExtensionAPI) {
  return LgtmPiExtensionService.register({ pi });
}
