...
import type { OpenReviewOptions } from "../../modules/review/server/server.ts";
import { LgtmPiExtensionServiceBuilder } from "./pi.ts";

describe("LgtmPiExtensionService", () => {
  it("registers discoverable sequential tools and returns review results with task context", async () => {
    ...
    const Extension = LgtmPiExtensionServiceBuilder({
      config: {},
      deps: {
