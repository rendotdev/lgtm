...
export type McpRuntimeDependencies = {
  collectGitReview: typeof GitReviewService.collect;
...
const defaultDependencies: McpRuntimeDependencies = {
  collectGitReview: GitReviewService.collect.bind(GitReviewService),
  finishReview,
  openReview,
  stopReview,
  waitForReview,
...
function createMcpToolHandlerImplementation(
...
async function runMcpServerImplementation() {
  const lines = createInterface({ input: process.stdin, crlfDelay: Number.POSITIVE_INFINITY });
  function send(value: unknown) {
    return process.stdout.write(`${JSON.stringify(value)}\n`);
  }
  const server = createMcpMessageHandlerImplementation(send);
...
function createMcpMessageHandlerImplementation(
  send: (value: unknown) => void,
  callTool = createMcpToolHandlerImplementation(),
) {
...
export const { McpEntrypointSingleton, McpEntrypointSingletonBuilder } = build().singleton(
  "McpEntrypointSingleton",
  {
    build() {
      return {
        createMcpMessageHandler: createMcpMessageHandlerImplementation,
        createMcpToolHandler: createMcpToolHandlerImplementation,
        runMcpServer: runMcpServerImplementation,
      };
    },
  },
);

export const { createMcpMessageHandler, createMcpToolHandler, runMcpServer } =
  McpEntrypointSingleton;
