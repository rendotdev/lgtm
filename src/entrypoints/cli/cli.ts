...
import {
  ...
  serveReviewApp,
  stopReviews,
} from "../../modules/review/server/server.ts";
import { GitReviewService } from "../../modules/review/git/git.ts";
import type { ReviewPointer } from "../../modules/review/review/review.ts";
import { AgentInstallSingleton } from "../../modules/install/install.ts";
import { AgentInstallerService, AgentUpdaterService } from "../../modules/install/agent/agent.ts";
import { CliUpdaterService } from "../../modules/install/cli-update/cli-update.ts";
import { runMcpServer } from "../mcp/mcp.ts";
import { JsonReviewInputSingleton, ReviewGroupsInputSingleton } from "./json-input/json-input.ts";
import { CliCommandRunnerServiceBuilder } from "./runner/runner.ts";
import { CommandUiRendererService } from "./ui/ui.tsx";
...
const reviewSessionId = configuredReviewSessionId
  ? ReviewIdentifierSingleton.sanitizePathSegment({ value: configuredReviewSessionId })
  : undefined;
const cancellation = new AbortController();
let cancelling = false;
let commandErrorRendered = false;
const CliCommandRunnerService = CliCommandRunnerServiceBuilder({
  config: { jsonOutput },
  deps: {
...
function formatPointer(pointer: ReviewPointer) {
  return CommandUiRendererService.formatDetail({
    lines: [
...
async function readReviewGroups(path: string | undefined) {
  ...
  return ReviewGroupsInputSingleton.parse({
...
async function main() {
  ...
  if (isSetupCommand) {
    const targetValue = takeOption("--target") ?? "all";
    const target = AgentInstallSingleton.parseTarget({ value: targetValue });
    if (!target) {
      throw new Error("setup --target must be one of: all, pi, claude, codex.");
    }
    const plan = AgentInstallSingleton.createInstallPlan({ target });
    if (takeFlag("--dry-run")) {
      await CliCommandRunnerService.run({
        ...
        renderSuccess: function renderSuccess(result) {
          return CommandUiRendererService.formatIntegrationResult(result);
        },
      });
      return;
    }
    await CliCommandRunnerService.run({
      ...
      execute: async () => ({
        ...
        target,
        steps: await AgentInstallerService.install({ target }),
      }),
      renderSuccess: function renderSuccess(result) {
        return CommandUiRendererService.formatIntegrationResult(result);
      },
  ...
  if (command === "update") {
    ...
    const plan = AgentInstallSingleton.createUpdatePlan({ target });
    if (takeFlag("--dry-run")) {
      await CliCommandRunnerService.run({
        ...
        execute: async () => ({
          ...
          target,
          steps: plan,
          cli: await CliUpdaterService.plan({}),
          dryRun: true,
        }),
        renderSuccess: function renderSuccess(result) {
          return CommandUiRendererService.formatIntegrationResult(result);
    ...
    await CliCommandRunnerService.run({
      ...
      execute: async function execute(report) {
        report.complete({ label: `Current version: ${CliUpdaterService.getCurrentVersion({})}` });
        report("Checking for updates");
        const cliPlan = await CliUpdaterService.plan({});
        let cli;
        if (cliPlan.status === "ready") {
          ...
          report("Updating CLI");
          cli = await CliUpdaterService.update({ plan: cliPlan });
          ...
          report.complete({
            ...
            detail: CommandUiRendererService.formatCommandOutputGroups({
              steps: [cli.step],
              outputs: [cli.output],
            }),
            mutedDetail: true,
        ...
        const integrations = await AgentUpdaterService.update({
          target,
          onUpdate: function onUpdate(event) {
            const name = CommandUiRendererService.formatIntegrationName({ target: event.target });
    ...
    if (reviewCommand === "git") {
      ...
      await CliCommandRunnerService.run({
        ...
        execute: async (report) => {
          ...
          const collection = await GitReviewService.collect({
            cwd,
            remote,
            remoteCwd,
            sessionId: reviewSessionId,
    ...
    if (reviewCommand === "json") {
      ...
      await CliCommandRunnerService.run({
        ...
        execute: async (report) => {
          ...
          const input = JsonReviewInputSingleton.parse({
