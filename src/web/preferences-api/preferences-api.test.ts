...
import { PreferencesApiServiceBuilder } from "./preferences-api.ts";
...
describe("PreferencesApiService", () => {
  it("binds fetch to the browser global", async () => {
    ...
    await expect(
      PreferencesApiServiceBuilder({ config: {}, deps: { fetch } }).get({}),
    ).resolves.toEqual({
      diffStyle: "unified",
      lineWrap: false,
      sidebarWidth: 256,
  ...
  it("updates preferences through a JSON mutation", async () => {
    ...
    await expect(
      PreferencesApiServiceBuilder({ config: {}, deps: { fetch } }).update({
        preferences: {
          diffStyle: "unified",
          lineWrap: false,
          sidebarWidth: 288,
