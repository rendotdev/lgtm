...
import { ReviewClipboardCopyServiceBuilder } from "./review-clipboard-copy.ts";

describe("ReviewClipboardCopyService", () => {
  it("starts before writing and finishes only after the clipboard resolves", async () => {
    ...
    const events: string[] = [];
    const ClipboardCopy = ReviewClipboardCopyServiceBuilder({ config: {}, deps: { writeText } });
  ...
  it("finishes the loading state when clipboard access fails", async () => {
    const events: string[] = [];
    const ClipboardCopy = ReviewClipboardCopyServiceBuilder({
      config: {},
      deps: {
