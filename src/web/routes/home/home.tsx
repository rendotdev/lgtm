...
} from "../../../modules/preferences/preferences.ts";
import { ReviewHandoffSingleton } from "../../../modules/review/handoff/handoff.ts";
import type {
  DocumentComment,
  DocumentSource,
  ReviewComment,
  ...
  ReviewPayload,
  ReviewSourceFile,
} from "../../../modules/review/review/review.ts";
import { CommentDraftService } from "../../comment-draft/comment-draft.ts";
import { DocumentCodeHighlighterService } from "../../document-code-highlighter/document-code-highlighter.ts";
import { DocumentMarkdownNavigationSingleton } from "../../document-markdown-navigation/document-markdown-navigation.ts";
import { FileSearchSingleton } from "../../file-search/file-search.ts";
import { PreferencesApiService } from "../../preferences-api/preferences-api.ts";
import { ReviewApiService, type ReviewAppState } from "../../review-api/review-api.ts";
import { ReviewClipboardCopyService } from "../../review-clipboard-copy/review-clipboard-copy.ts";
import {
  ReviewCommentInteractionService,
  type DiffScrollAnchor,
  type ElementScrollAnchor,
} from "../../review-comment-interaction/review-comment-interaction.ts";
import { ReviewDiffPresentationService } from "../../review-diff-presentation/review-diff-presentation.ts";
import { ReviewFileNavigationService } from "../../review-file-navigation/review-file-navigation.ts";
import { ReviewGroupPresentationSingleton } from "../../review-group-presentation/review-group-presentation.ts";
import { ReviewPresentationService } from "../../review-presentation/review-presentation.ts";
import { ToastNotificationsService } from "../../toast-notifications/toast-notifications.ts";
import { useReviewServerMonitor } from "../../review-server-monitor/review-server-monitor.ts";
import { ReviewWindowTitleSingleton } from "../../window-title/window-title.ts";
import { CommentEditorComponent } from "./components/comment-editor/comment-editor.tsx";
import { HomeContentComponent } from "./components/content/content.tsx";
import { HomeFooterComponent } from "./components/footer/footer.tsx";
import { HomeSidebarComponent } from "./components/sidebar/sidebar.tsx";
import { ReviewCodeFrameComponent } from "./components/review-code-frame/review-code-frame.tsx";
import { HomeTemplateComponent } from "./template/template.tsx";
...
function HomeRouteView() {
  ...
  const [displayedSidebarWidth, setDisplayedSidebarWidth] = useState(
    LgtmPreferencesSingleton.defaults.sidebarWidth,
  );
  ...
  const preferencesQuery = useQuery({
    ...
    queryFn: () => PreferencesApiService.get({}),
    staleTime: Number.POSITIVE_INFINITY,
  });
  const preferencesMutation = useMutation({
    mutationFn: (preferences: LgtmPreferences) => PreferencesApiService.update({ preferences }),
    ...
    onError: (error, _preferences, context) => {
      queryClient.setQueryData(
        ["preferences"],
        context?.previousPreferences ?? LgtmPreferencesSingleton.defaults,
      );
      ToastNotificationsService.preferencesNotSaved({ error });
    },
  ...
  const diffTheme = ReviewDiffPresentationService.resolveTheme({ resolvedTheme });
  ...
  useEffect(() => {
    ...
    ToastNotificationsService.preferencesUnavailable({});
  ...
  useEffect(() => {
    ...
    lastSavedSignature.current = ReviewPresentationService.meaningfulSignature({
      review: nextState.review,
    });
    document.title = ReviewWindowTitleSingleton.format({
      cwd: nextState.payload.cwd,
      name: nextState.payload.name,
    });
    setCollapsedFileIds(
      ReviewPresentationService.initialCollapsedFileIds({
        state: nextState,
        fileExpansion,
  ...
  useEffect(() => {
    ...
    ToastNotificationsService.reviewUnavailable({});
  }, [reviewStateQuery.error]);

  useReviewServerMonitor({
    getCommentCount: function getCommentCount() {
      const review = latestReviewRef.current;
      return review === null ? 0 : ReviewPresentationService.commentCount({ review });
    },
  });

  useEffect(function trackPointerInteractions() {
    ReviewCommentInteractionService.installPointerTracking({ node: window, phase: "mount" });
    return function stopTrackingPointerInteractions() {
      ReviewCommentInteractionService.installPointerTracking({ node: window, phase: "unmount" });
    };
  }, []);
  ...
  const saveDebouncer = useAsyncDebouncer(
    async (review: ReviewJson) => {
      const signature = ReviewPresentationService.meaningfulSignature({ review });
    ...
    },
    {
      ...
      onError: () => {
        ...
        ToastNotificationsService.commentsNotSaved({});
      },
    },
  ...
  const addComment = useCallback(
    (file: ReviewSourceFile, selectedRange: SelectedLineRange, selectedTextOverride?: string) => {
      ...
      const selectedText = selectedTextOverride?.trim()
        ...
        : ReviewCommentInteractionService.selectedText({ file, side, startLine, endLine });
      const now = new Date().toISOString();
      const comment: ReviewComment = {
        id: ReviewCommentInteractionService.createId({}),
      ...
      commitReview((review) =>
        ReviewPresentationService.updateFile({
          review,
          fileLocation: file.location,
          updater: (reviewFile) => ({
            ...reviewFile,
  ...
  async function copyReviewHandoff(params: { text: string; status: string }): Promise<boolean> {
    return await ReviewClipboardCopyService.copy({
      text: params.text,
  ...
  async function finishReview(decision: "approved" | "changes_requested") {
    ...
    const review = CommentDraftService.applyToReview({
      review: latestReviewRef.current ?? state.review,
    });
    const hasDraftComments = ReviewPresentationService.commentCount({ review }) > 0;
    ...
    const didCopyHandoff = await copyReviewHandoff({
      text: ReviewHandoffSingleton.clipboardText({ decision: resolvedDecision, review }),
    ...
    if (!didCopyHandoff) {
      ...
      hideBusyIndicatorDebouncer.cancel();
      setShowBusyIndicator(false);
      ToastNotificationsService.copyFailed({});
      return;
    }
    ...
    try {
      ...
      const didCopyComments = await copyReviewHandoff({
        text: ReviewHandoffSingleton.fallbackText({ review }),
      ...
      if (didCopyComments) {
        ToastNotificationsService.commentsCopied({});
      } else {
        ToastNotificationsService.commentsKeptInTab({});
      }
      return;
    }
    try {
      ...
      ToastNotificationsService.reviewNotFinished({});
    }
  }
  ...
  async function cancelReview() {
    ...
    try {
      ...
      setIsFinishing(false);
      ToastNotificationsService.cancelFailed({});
    }
  }

  const primaryDecision =
    state && ReviewPresentationService.commentCount({ review: state.review }) > 0
      ? "changes_requested"
      : "approved";
  ...
  const commentCount = ReviewPresentationService.commentCount({ review });
...
function DiffReviewList(props: {
  ...
  const [selectedFileLocation, setSelectedFileLocation] = useState(() =>
    ReviewFileNavigationService.read({ search: window.location.search }),
  );
  const [scrollMargin, setScrollMargin] = useState(0);
  const sidebarFiles = useMemo(
    () => FileSearchSingleton.search({ files: props.payload.files, query: deferredFileQuery }),
    [deferredFileQuery, props.payload.files],
  );
  const sidebarGroups = useMemo(
    () =>
      ReviewGroupPresentationSingleton.build({ files: sidebarFiles, groups: props.payload.groups }),
  ...
  const reviewGroups = useMemo(
    () =>
      ReviewGroupPresentationSingleton.build({
  ...
  useEffect(
    function prepareFileSearchIndex() {
      FileSearchSingleton.prepare({ files: props.payload.files });
    },
    [props.payload.files],
  );
  ...
  const scrollToFile = useCallback(
    function scrollToFile(params: { fileId: string; updateUrl: boolean }) {
      ...
      if (params.updateUrl) {
        const currentLocation = ReviewFileNavigationService.read({
  ...
  useEffect(() => {
    function updateSelectedFileFromUrl() {
      setSelectedFileLocation(ReviewFileNavigationService.read({ search: window.location.search }));
    }
  ...
  function LoadedDocumentMarkdownRenderer(props: {
    ...
    return (
      <ReactMarkdown
        ...
        rehypePlugins={[DocumentMarkdownNavigationSingleton.buildHeadingIdPlugin({})]}
  ...
  const { capture: captureDocumentScrollAnchor, stabilize: stabilizeDocumentScrollAnchor } =
    useScrollAnchorStabilizer<ElementScrollAnchor>({
      ...
      restore: function restoreDocumentScrollAnchor(anchor) {
        ReviewCommentInteractionService.restoreElementScrollAnchor({ anchor });
      },
    });
  const captureScrollAnchor = useCallback(
    function captureScrollAnchor(element: HTMLElement) {
      ...
      captureDocumentScrollAnchor(
        ReviewCommentInteractionService.captureElementScrollAnchor({ element, scrollElement }),
      );
    },
    [captureDocumentScrollAnchor],
  );
...
const DocumentMarkdownTree = React.memo(function DocumentMarkdownTree(
  ...
  const components = useMemo<Components>(
    () => ({
      ...
      a: ({ node: _node, children, ...elementProps }) => {
        const linkAttributes = DocumentMarkdownNavigationSingleton.linkAttributes({
  ...
  function handleMouseUp() {
    window.setTimeout(() => {
      ...
      const textSelection = ReviewCommentInteractionService.currentTextSelection({ root });
      if (!textSelection) {
        return;
      }
      ...
      const selectedDocumentLines =
        startBlock === endBlock
          ? ReviewCommentInteractionService.selectedDocumentLineRange({ root: startBlock, range })
          : null;
...
function DocumentCodeBlock(props: DocumentCodeBlockProps) {
  ...
  const code =
    typeof codeElementProps?.children === "string"
      ...
      : "";
  const language = DocumentCodeHighlighterService.languageFromClassName({ className });
  ...
  const fileOptions = useMemo<NonNullable<FileProps<CommentAnnotationMetadata>["options"]>>(
    () => ({
      ...
      unsafeCSS: ReviewDiffPresentationService.fileOptions({}).unsafeCSS,
      ...
      onPostRender: function onPostRender(node, instance, phase) {
        ReviewCommentInteractionService.installRowSelection({
          node,
          phase,
          renderer: instance,
...
const ReviewFileDiff = React.memo(function ReviewFileDiff(props: ReviewFileDiffProps) {
  ...
  const { capture: captureDiffScrollAnchor, stabilize: stabilizeDiffScrollAnchor } =
    useScrollAnchorStabilizer<DiffScrollAnchor>({
      ...
      restore: function restoreDiffScrollAnchor(anchor) {
        ReviewCommentInteractionService.restoreDiffScrollAnchor({ anchor });
      },
    });
  const captureScrollAnchor = useCallback(
    function captureScrollAnchor(range: SelectedLineRange) {
      ...
      captureDiffScrollAnchor(
        ReviewCommentInteractionService.captureDiffScrollAnchor({ node, range }),
      );
    },
  ...
  >(() => {
    const presentationOptions = ReviewDiffPresentationService.fileOptions({});
    return {
      ...presentationOptions,
...
export const { HomeRoute, HomeRouteBuilder } = build().component("HomeRoute", HomeRouteView);
