...
export const { useReviewLineSelection, useReviewLineSelectionBuilder } = build().hook(
  "useReviewLineSelection",
  {
    ...
    build({ deps }) {
      return function useReviewLineSelection(_hookParams: {}) {
        const [selectedLines, setSelectedLines] = deps.useState<
        ...
        const clearSelectedLines = deps.useCallback(
          function clearSelectedLines(expectedRange?: SelectedLineRange) {
            ...
            clearFrameRef.current = deps.requestAnimationFrame(
              function releaseControlledSelection() {
                clearFrameRef.current = null;
