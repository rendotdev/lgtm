...
import { ReviewCommentInteractionService } from "../../../../review-comment-interaction/review-comment-interaction.ts";

const iconSize = 14;
const iconStrokeWidth = 1.5;

function CommentEditorView(props: {
  id: string;
  value: string;
  active: boolean;
  ...
  const form = useForm({
    defaultValues: {
      comment: CommentDraftService.value({ id: props.id, fallback: props.value }),
    },
  ...
  function finishComment(value: string) {
    ...
    if (isEmptyComment) {
      ReviewCommentInteractionService.finishAfterPointerInteraction({
        callback: function finishEmptyComment() {
          CommentDraftService.finish({
            id: props.id,
            value,
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ReviewCommentInteractionService.resizeTextarea({
  ...
  function handleClearComment() {
    setIsFocused(false);
    CommentDraftService.remove({ id: props.id });
    props.onDelete();
  }
...
export const { CommentEditorComponent, CommentEditorComponentBuilder } = build().component(
