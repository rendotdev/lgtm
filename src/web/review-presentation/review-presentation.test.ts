...
import { ReviewPresentationServiceBuilder } from "./review-presentation.ts";
...
describe("ReviewPresentationService", () => {
  it("counts meaningful comments and ignores empty drafts", () => {
    const Presentation = ReviewPresentationServiceBuilder({
      config: { defaultCollapsedChangedLineThreshold: 500 },
      deps: { now: () => new Date("2026-01-02T00:00:00.000Z") },
    });
    const value = review({
      files: [
        {
