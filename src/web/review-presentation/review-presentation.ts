...
import type {
  FileExpansion,
  FileExpansionOverride,
} from "../../modules/preferences/preferences.ts";
import type { ReviewFile, ReviewJson } from "../../modules/review/review/review.ts";
import type { ReviewAppState } from "../review-api/review-api.ts";

export const { ReviewPresentationService, ReviewPresentationServiceBuilder } = build().service(
  "ReviewPresentationService",
  {
    config: { defaultCollapsedChangedLineThreshold: 500 },
    deps: {
      now: function now() {
        return new Date();
      },
    ...
    build({ config, deps }) {
      function commentCount(params: { review: ReviewJson }): number {
      ...
      function meaningfulSignature(params: { review: ReviewJson }): string {
        if (params.review.kind === "document") {
      ...
      function updateFile(params: {
        ...
        return { ...params.review, updatedAt: deps.now().toISOString(), files };
      }

      function baseCollapsedFileIds(params: {
        ...
        return new Set(
          params.state.payload.files
            .filter(function shouldCollapseFile(file) {
              const reviewFile = reviewFileByLocation.get(file.location);
              return (
                file.added + file.removed >= config.defaultCollapsedChangedLineThreshold &&
      ...
      function initialCollapsedFileIds(params: {
        state: ReviewAppState;
        fileExpansion: FileExpansion;
        ...
        const collapsedFileIds = baseCollapsedFileIds({
          state: params.state,
      ...
      return { commentCount, initialCollapsedFileIds, meaningfulSignature, updateFile };
    },
  },
);
