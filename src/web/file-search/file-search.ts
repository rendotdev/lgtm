...
export const { FileSearchSingleton, FileSearchSingletonBuilder } = build().singleton(
  "FileSearchSingleton",
  {
    build() {
      const indexes = new WeakMap<ReviewSourceFile[], IndexedFile[]>();

      function prepare(params: { files: ReviewSourceFile[] }): void {
        indexFiles(params.files);
      }

      function search(params: { files: ReviewSourceFile[]; query: string }): ReviewSourceFile[] {
      ...
      function scoreSubsequence(query: string, candidate: string): number | null {
        ...
        return (
          420 + query.length * 18 + adjacencyBonus + boundaryBonus - gapPenalty - lengthPenalty
        );
      }
