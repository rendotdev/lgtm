...
import { DocumentCodeHighlighterServiceBuilder } from "./document-code-highlighter.ts";

function highlighter(params: { codeToHtml?: typeof codeToHtml } = {}) {
  return DocumentCodeHighlighterServiceBuilder({
    config: {},
    deps: {
...
describe("DocumentCodeHighlighterService", () => {
  it("reads fenced languages and treats patch blocks as diffs", () => {
    const DocumentCodeHighlighterService = highlighter();

    expect(
      DocumentCodeHighlighterService.languageFromClassName({ className: "language-typescript" }),
    ).toBe("typescript");
    expect(
      DocumentCodeHighlighterService.languageFromClassName({ className: "language-patch" }),
    ).toBe("diff");
    expect(DocumentCodeHighlighterService.languageFromClassName({})).toBe("text");
  });
  ...
  it("caches repeated highlighted blocks", async () => {
    ...
    const DocumentCodeHighlighterService = highlighter({ codeToHtml: codeToHtmlSpy });

    await DocumentCodeHighlighterService.highlight({
      code: "const value = 1;",
  ...
  it("caches the same code separately when its document line changes", async () => {
    ...
    await DocumentCodeHighlighterService.highlight({ code: "value", sourceStartLine: 2 });
    await DocumentCodeHighlighterService.highlight({ code: "value", sourceStartLine: 3 });
