...
import { DocumentMarkdownNavigationSingleton } from "./document-markdown-navigation.ts";

describe("DocumentMarkdownNavigationSingleton", () => {
  it("keeps document anchors in the current tab and opens other links externally", () => {
    expect(DocumentMarkdownNavigationSingleton.linkAttributes({ href: "#details" })).toEqual({});
    expect(
      DocumentMarkdownNavigationSingleton.linkAttributes({
  ...
  it("adds stable, unique IDs to rendered headings", () => {
    ...
    const transform = DocumentMarkdownNavigationSingleton.buildHeadingIdPlugin({})();
