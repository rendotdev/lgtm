...
export const { DocumentMarkdownNavigationSingleton, DocumentMarkdownNavigationSingletonBuilder } =
  build().singleton("DocumentMarkdownNavigationSingleton", {
    build() {
      function buildHeadingIdPlugin(params: {}) {
        void params;
        return createHeadingIdTransformer;
      }

      function linkAttributes(params: { href?: string }) {
      ...
      return { buildHeadingIdPlugin, linkAttributes };
    },
  });
