...
import { ReviewGroupPresentationSingleton } from "./review-group-presentation.ts";
...
describe("ReviewGroupPresentationSingleton", () => {
  it("returns one untitled group for a flat review", () => {
    expect(ReviewGroupPresentationSingleton.build({ files, groups: undefined })).toEqual([
      { title: null, files },
    ]);
  });
