...
import { CommentDraftServiceBuilder } from "./comment-draft.ts";

function commentDraft() {
  return CommentDraftServiceBuilder({
...
describe("CommentDraftService", () => {
  ...
  it("keeps rapid input local until the deferred review sync runs", () => {
    ...
    const CommentDraftService = CommentDraftServiceBuilder({
      config: { syncWaitMs: 250 },
      deps: {
        clearTimeout: vi.fn(),
        now: () => new Date("2026-07-18T00:00:00.000Z"),
    ...
    CommentDraftService.update({ id: "comment", value: "Fast typing", onSync });

    expect(CommentDraftService.value({ id: "comment", fallback: "" })).toBe("Fast typing");
  ...
  it("applies an unsynchronized draft before finishing a review", () => {
    const CommentDraftService = commentDraft();
    ...
    CommentDraftService.update({ id: "comment", value: "Unsynchronized draft", onSync: vi.fn() });

    const reviewWithDraft = CommentDraftService.applyToReview({ review });
